package kb_cytoscape::kb_cytoscapeServer;
...
sub call_method {
    ...
    my $ctx = kb_cytoscape::kb_cytoscapeServerContext->new(
    ...
    # Service kb_cytoscape requires authentication.
    ...
    if ( $method_auth eq 'none' ) {
        ...
        if ( !$token && $method_auth eq 'required' ) {
            $self->exception( 'PerlError',
                "Authentication required for kb_cytoscape but no authentication header was passed"
            );
        }
    ...
    my @result;
    {
        ...
        my $stderr = kb_cytoscape::kb_cytoscapeServerStderrWrapper->new( $ctx, $get_time );
        $ctx->stderr( $stderr );
...
sub get_method {
    ...
    if ( !$self->valid_methods->{ $method } ) {
        $self->exception( 'NoSuchMethod',
             "'$method' is not a valid method in service kb_cytoscape."
        );
    }
...
package kb_cytoscape::kb_cytoscapeServerContext;
...
kb_cytoscape::kb_cytoscapeServerContext
...
package kb_cytoscape::kb_cytoscapeServerStderrWrapper;
...
sub new {
    ...
    my $dest        = $ENV{ KBRPC_ERROR_DEST }
        if exists $ENV{ KBRPC_ERROR_DEST };
    my $tag         = $ENV{ KBRPC_TAG }
        if exists $ENV{ KBRPC_TAG };
...
unless ( caller ) {
    ...
    my @dispatch;
    {
        ...
        my $obj = kb_cytoscape::kb_cytoscapeImpl->new;
        push( @dispatch, 'kb_cytoscape' => $obj );
    }
    my %headers = (
        Authorization   => $token,
        CLI             => "1",
